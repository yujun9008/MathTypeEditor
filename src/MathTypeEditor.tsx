...
const MathEditor: React.FC<MathEditorProps> = ({ defaultValue, onChange, style }) => {
    ...
    const initListeners = () => {
      ...
      if (mf) {
        ...
        const onPaste = (e: ClipboardEvent) => {
          ...
          const fixLatex = (latex: string): string => {
            let current = latex
  ...
  const onLatexChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
          ...
          {editMode === 'mathlive'
            ? (
              <div className="editKatax">
                <p className="title">公式编辑区域（支持直接粘贴公式）：</p>
                <math-field ref={mfRef} className="mathField" />
              </div>
            )
            : (
              <>
                ...
                <div className="previewKatax">
                  <p className="previewTitle">公式预览：</p>
                  <div
